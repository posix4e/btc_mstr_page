...
Usage: python3 update_data_mnav.py /path/to/excel_file.xlsx
...
def convert_excel_to_json(excel_path):
    """Convert Excel file to JSON format for the website with mNAV data"""
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 update_data_mnav.py /path/to/excel_file.xlsx")
    ...
    try:
        ...
        output_file = 'data_with_mnav.json' if has_mnav else 'data.json'
        with open(output_file, 'w') as f:
            json.dump(data, f, indent=2)

        print(f"✅ Successfully updated {output_file} with {len(data)} monthly data points")
